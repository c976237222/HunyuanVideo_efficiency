...
from hyvideo.vae.vae import DecoderCausal3D, DecoderOutput, DiagonalGaussianDistribution, EncoderCausal3D
from diffusers.utils import BaseOutput
def _apply_t_ops_config_to_vae(vae, t_ops_config: dict):
    """
...
class AutoencoderKLCausal3D(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    ...
    def __init__(
        ...
        super().__init__()
        base_path = "/home/hanling/HunyuanVideo_efficiency/dlfr_generation/"
        self.time_compression_ratio = time_compression_ratio
        self._used_tile_ratios_encode = []
        try:
            with open(f"{base_path}config_1x.json", "r") as f:
                self.t_ops_config_1x = json.load(f)
            with open(f"{base_path}config_2x.json", "r") as f:
                self.t_ops_config_2x = json.load(f)
            with open(f"{base_path}config_4x.json", "r") as f:
                self.t_ops_config_4x = json.load(f)
            logger.info("Loaded 1x/2x/4x T-ops configs for dynamic decode.")
        except FileNotFoundError:
            logger.warning("Not found config_1x.json / config_2x.json / config_4x.json, dynamic decode won't work.")
            self.t_ops_config_1x = None
            self.t_ops_config_2x = None
            self.t_ops_config_4x = None

        self._current_ratio_key = None
    ...
    def _switch_config(self, ratio_key: str):
        if ratio_key == self._current_ratio_key:
            # 已经是当前 config，就不再重复注入
            return

        # 选择对应 config
        if ratio_key == 1 and self.t_ops_config_1x is not None:
            cfg = self.t_ops_config_1x
        elif ratio_key == 2 and self.t_ops_config_2x is not None:
            cfg = self.t_ops_config_2x
        elif ratio_key == 4 and self.t_ops_config_4x is not None:
            cfg = self.t_ops_config_4x
        else:
            raise ValueError(f"Unknown ratio_key={ratio_key} or config is None!")

        # 注入
        logger.info(f"Switching T-ops config -> {ratio_key} ...")
        # 这里要调用你自己的注入函数
        _apply_t_ops_config_to_vae(self, cfg)

        self._current_ratio_key = ratio_key
    ...
    def spatial_tiled_decode(self, z: torch.FloatTensor, return_dict: bool = True,  CR = None) -> Union[DecoderOutput, torch.FloatTensor]:
        r"""
    ...
    def temporal_tiled_decode(self, z: torch.FloatTensor, return_dict: bool = True, ratio_list=None,) -> Union[DecoderOutput, torch.FloatTensor]:
        B, C, T, H, W = z.shape
        ...
        i = 0
        row = []
        num_tiles = len(ratio_list)
        scale_map = {4: 1, 8: 0.5, 16: 0.25}
        _CR = {4: 1, 8: 2, 16: 4}
        for tile_ind in range(num_tiles):
            cur_ratio = _CR[ratio_list[tile_ind]]
            tile_latent_size = int(self.tile_latent_min_tsize*scale_map[ratio_list[tile_ind]]*0.75)
            ...
            if tile_ind < num_tiles - 1:
                next_ratio = _CR[ratio_list[tile_ind + 1]]
                ...
                if cur_ratio<next_ratio and cur_blend_extent!=0:
                    extend=F.interpolate(extend, size=(cur_blend_extent, H, W), mode='nearest')
